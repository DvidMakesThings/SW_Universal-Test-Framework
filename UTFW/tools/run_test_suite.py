...
class TestSuiteRunner:
    ...
    def _rel_href(self, target: Path, base_dir: Path) -> str:
        """
        Return a POSIX-style relative path from base_dir to target.
        Falls back to filename if a relative path cannot be computed.
        """
        try:
    ...
    def _find_test_report(self, test_name: str, test_path: str) -> Optional[Path]:
        """
        Find the HTML report generated by an individual test.
        Tests typically create reports in: Reports/report_<test_name>/<test_name>_report.html
        """
        if not test_path:
            return None

        # Try common patterns for test report locations
        test_basename = Path(test_path).stem

        # When running via suite with -r flag, tests create their reports in:
        # <reports_dir>/report_<test_basename>/<test_basename>_report.html
        report_dir_name = f"report_{test_basename}"

        # Pattern candidates to search
        report_candidates = [
            # Pattern 1: report_<test_basename>/<test_basename>_report.html (most common)
            self.reports_dir / report_dir_name / f"{test_basename}_report.html",
            # Pattern 2: <test_basename>/<test_basename>_report.html
            self.reports_dir / test_basename / f"{test_basename}_report.html",
            # Pattern 3: report_<test_basename>/report.html
            self.reports_dir / report_dir_name / "report.html",
            # Pattern 4: Direct in reports_dir
            self.reports_dir / f"{test_basename}_report.html",
        ]

        # Also search for any HTML files in the test's reports subdirectory
        test_reports_dir = self.reports_dir / report_dir_name
        if test_reports_dir.exists() and test_reports_dir.is_dir():
            for html_file in test_reports_dir.glob("*.html"):
                if html_file.is_file() and "_report" in html_file.name:
                    # Prioritize files with "_report" in the name
                    report_candidates.insert(0, html_file)
    ...
    def _generate_html_report(self, suite_name: str, start_time: datetime, duration: float):
        ...
        .test-link{color:#9ad0ff;font-weight:600}
        .test-link:hover{text-decoration:underline}
        .report-badge{display:inline-block;padding:2px 6px;margin-left:6px;border-radius:8px;background:#1a3a52;color:#7dd3fc;font-size:11px;border:1px solid #2563eb}
        """
        ...
        for idx, result in enumerate(self.results, 1):
            ...
            # Try to find the test's individual HTML report
            test_report = self._find_test_report(result['name'], result.get('path', ''))

            html_lines.append(f"<tr class='{status_class}'>")
            html_lines.append(f"<td>{idx}</td>")

            # Make test name clickable if report exists
            html_lines.append("<td>")
            if test_report:
                rel_path = self._rel_href(test_report, report_file.parent)
                html_lines.append(
                    f"<a href='{html.escape(rel_path)}' target='_blank' class='test-link'>"
                    f"{html.escape(result['name'])}</a>"
                    f"<span class='report-badge'>📄 Report</span>"
                )
            else:
                html_lines.append(html.escape(result['name']))
            html_lines.append("</td>")
