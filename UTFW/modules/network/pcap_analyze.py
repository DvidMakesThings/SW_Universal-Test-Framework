...
def _match_payload_patterns(payload: bytes, patterns: List[Dict[str, Any]]) -> Optional[str]:
    """
...
def _run_tshark_fields(pcap_path: str, display_filter: Optional[str], extra_fields: Optional[List[str]] = None) -> Tuple[str, str, int]:
    _ensure_tshark()
    fields = list(_TS_FIELDS_BASE)
    if extra_fields:
        fields.extend(extra_fields)
    cmd = ["tshark", "-r", pcap_path, "-T", "fields", "-E", "header=n", "-E", "separator=\t", "-E", "occurrence=f"]
    for f in fields:
        cmd += ["-e", f]
    if display_filter:
...
def _run_tshark_vlan_stack(pcap_path: str, display_filter: Optional[str]) -> List[List[Tuple[int, Optional[int]]]]:
    """
    ...
    cmd = [
        ...
        "-E", "header=n", "-E", "separator=\t", "-E", "occurrence=a",
...
def read_PCAPFrames(name: str,
                    pcap_path: str,
                    display_filter: Optional[str] = None) -> TestAction:
    """
...
def analyze_PCAP(name: str,
    ...
    def execute():
        ...
        if expect_count is not None and len(frames) != int(expect_count):
            raise PCAPAnalyzeError(f"Expected {expect_count} frames after filter, got {len(frames)}")
...
def pcap_checkFrames(name: str,
                     pcap_path: str,
                     *,
                     display_filter: Optional[str] = None,
                     expect_count: Optional[int] = None,
                     expected_frames: Optional[List[Dict[str, Any]]] = None,
                     ordered: bool = True) -> TestAction:
    """
    ...
    def execute():
        _log(f"[PCAP-EXPECT] checkFrames start path={pcap_path} filter={display_filter or 'none'} "
        ...
        if not expected_frames:
            _log(f"[PCAP-EXPECT] no expected_frames specified, returning parsed list (n={len(frames)})")
            return frames

        if ordered:
            if len(expected_frames) > len(frames):
                raise PCAPAnalyzeError(f"Expected {len(expected_frames)} frames (ordered), got {len(frames)}")
            for idx, exp in enumerate(expected_frames):
                msg = _frame_satisfies(frames[idx], exp)
                if msg:
                    raise PCAPAnalyzeError(f"Frame[{idx+1}] does not satisfy expectation: {msg}")
        ...
        _log(f"[PCAP-EXPECT] validated {len(expected_frames)} expected frame(s) "
