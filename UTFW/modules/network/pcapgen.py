    ...
    *,
    ...
    while offset < total:
        ...
        pkt = build_ipv4_packet(
            ...
            flags_df=False,
            flags_mf=more,
            frag_offset_units8=frag_off_units8,
            ttl=ttl,
            tos=tos,
        )
        packets.append(pkt)
...
def build_ethernet_frame(
    *,
    ...
    if total_size_including_fcs is not None:
        ...
        if want < min_len:
            raise PCAPGenError(
                "total_size_including_fcs smaller than header+payload+FCS"
            )
...
def pcap_create(
    ...
    *,
    ...
    def execute():
        ...
        if logger:
            logger.log(
                f"[PCAPGEN] pcap_create -> target={output_path} last_ts_ns={last_ts_ns} last_len={last_len} "
        ...
        if ipv4:
            ...
            if logger:
                logger.log(
                    f"[PCAPGEN] IPv4 build: src={ip_src} dst={ip_dst} proto={ip_protocol} "
            ...
            if ip_auto_fragment_payload_size and ip_auto_fragment_payload_size > 0:
                frags = fragment_ipv4_payload_auto(
                    src=ip_src,
                    dst=ip_dst,
                    full_payload=ip_payload_final,
                    protocol=ip_protocol,
                    frag_payload_size=int(ip_auto_fragment_payload_size),
                ...
                if logger:
                    logger.log(
                        f"[PCAPGEN] IPv4 auto-fragment -> {len(frags)} packets "
            ...
            if logger:
                logger.log(
                    f"[PCAPGEN] Ether frame len={len(eth)} et={ethertype} "
                    f"payload_len={len(payload_final)} fcs_xor=0x{int(fcs_xormask):08X}"
                )

        # Compute timestamps and append
        timestamps: List[int] = []
        for i, (frame_bytes, d_ns) in enumerate(
            zip(frames_to_write, deltas_ns), start=1
        ):
            if last_ts_ns is None and i == 1:
                ...
                if d_ns is not None:
                    ...
                    if not link_bps:
                        raise PCAPGenError(
                            "link_speed_bps required when using ifg_bytes"
                        )
                    prev_bits = _frame_wire_bits(prev_len)
                    add_ns = _ns_from_ifg_bytes(
                        prev_bits, int(ifg_bytes), float(link_bps)
                    )
                    current_ts = (
                        last_ts_ns
                        if (i == 1 and last_ts_ns is not None)
            ...
            if logger:
                ...
                logger.log(
                    f"[PCAPGEN] frame#{i} ts={timestamps[-1]} ns len={len(frame_bytes)} "
        ...
        if logger:
            logger.log(
                f"[PCAPGEN] appended {len(frames_to_write)} frame(s) -> {output_path} "
...
def pcap_from_spec_action(
    ...
    *,
    ...
    def execute():
        ...
        if logger:
            logger.log(
                f"[PCAPGEN] from-spec target={output_path} last_ts_ns={last_ts_ns} last_len={last_len} "
        ...
        for idx, spec in enumerate(frames_spec, start=1):
            ...
            if ipv4:
                ...
                if logger:
                    logger.log(
                        f"[PCAPGEN] spec#{idx} IPv4 src={ip_src} dst={ip_dst} proto={ip_protocol} "
                ...
                if ip_auto and ip_auto > 0:
                    ...
                    for fi, pkt in enumerate(frags, start=1):
                        ...
                        if logger:
                            logger.log(
                                f"[PCAPGEN] spec#{idx} auto-frag#{fi} len={len(eth)}"
                            )
                else:
                    pkt = build_ipv4_packet(
                        src=ip_src,
                ...
                if logger:
                    logger.log(
                        f"[PCAPGEN] spec#{idx} Ether frame len={len(eth)} et={ethertype} "
            ...
            this_link = _parse_link_speed_bps(
                spec.get("link_speed_bps", link_speed_bps)
            )

            for fi, (frame_bytes, d_ns) in enumerate(
                zip(frames_to_write, deltas_ns), start=1
            ):
                if last_ts_ns is None and not out_ts:
                    ...
                    if d_ns is not None:
                        ...
                        if not use_bps:
                            raise PCAPGenError(
                                "link_speed_bps required when using ifg_bytes in spec"
                            )
                        prev_bits = _frame_wire_bits(prev_len)
                        add_ns = _ns_from_ifg_bytes(
                            prev_bits, int(this_ifg), float(use_bps)
                        )
                        current_ts = (
                            last_ts_ns
                            if (not out_ts and last_ts_ns is not None)
                        ...
                        if use_bps:
                            ser_ns = int(
                                (_frame_wire_bits(prev_len) / float(use_bps)) * 1e9
                            )
                            current_ts = (
                                last_ts_ns
                                if (not out_ts and last_ts_ns is not None)
                ...
                if logger:
                    ...
                    crc_calc = _crc32_le(body_wo_fcs)
                    logger.log(
                        f"[PCAPGEN] spec#{idx} frame#{fi} ts={out_ts[-1]} ns len={len(frame_bytes)} "
        ...
        if logger:
            logger.log(
                f"[PCAPGEN] from-spec appended {len(out_frames)} frame(s) to {output_path} "
