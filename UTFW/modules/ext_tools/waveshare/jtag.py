# jtag.py
"""
UTFW Waveshare JTAG Module
============================
High-level JTAG test functions and TestAction factories for the
Waveshare USB TO UART/I2C/SPI/JTAG adapter (WCH CH347 chipset).

This module provides JTAG interface capabilities through the CH347
adapter by leveraging the bundled OpenOCD binary with CH347 support.
The CH347 in Mode 3/4 exposes a JTAG master interface supporting
standard IEEE 1149.1 operations.

The JTAG implementation delegates low-level TAP state machine control
...
OpenOCD is bundled in ext_tools/ch347/OpenOCD_CH347/ and ships with a
pre-configured ch347.cfg adapter configuration file.

All JTAG operations are logged using the UTFW logging system with full
...
def _read_idcode(timeout: float = OPENOCD_TIMEOUT,
    ...
    if not idcodes:
        if logger:
            logger.warning("  No JTAG devices found on chain")
        return None
...
def detect_device(timeout: float = OPENOCD_TIMEOUT,
    ...
    if logger:
        logger.info(f"  OpenOCD binary: {'✓' if info['openocd_available'] else '✗'} {OPENOCD_BIN}")
        logger.info(f"  CH347 config:   {'✓' if info['config_available'] else '✗'} {OPENOCD_CFG}")

    if not info['openocd_available'] or not info['config_available']:
        if logger:
            logger.warning("  ⚠ OpenOCD or config not available")
            logger.info("=" * 80)
        return info
    ...
    try:
        ...
        if logger:
            logger.warning(f"  ⚠ Chain scan failed: {e}")
...
def scan(
    ...
    def execute():
        ...
        if logger:
            logger.info(f"✓ JTAG chain scan complete ({len(idcodes)} device(s))")
...
def read_idcode(
    ...
    def execute():
        ...
        if expected_idcode is not None:
            ...
            if logger:
                logger.info(f"✓ JTAG IDCODE verified: {idcode}")
...
def detect(
    ...
    def execute():
        ...
        if logger:
            logger.info(f"✓ JTAG detection complete: {info['device_count']} device(s)")
